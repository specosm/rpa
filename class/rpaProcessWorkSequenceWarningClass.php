 ...
 * Time: 10:58
 */
include 'rpaProcessWorkSequenceWarningClassImpl.php';
class rpaProcessWorkSequenceWarningClass implements rpaProcessWorkSequenceWarningClassImpl
{
...
private $per_page;
    ...
    /**
     * rpaProcessWorkSequenceErrorClass constructor.
    ...
    public function Paginador()
    {
        ...
        $db=new conexion();
        $filtro='';
            $filtro.='WHERE id_rpa_process_work_warning= '.$this->getId();

        if ($this->getPage() != 1) $start = ($this->getPage()-1) * $this->getPerPage();
        else $start=0;

        $select = $db->query("SELECT * FROM rpa_process_work_warning $filtro LIMIT $start, $this->per_page "); // Select article list from $start

        $numArticles = $db->query("SELECT id FROM rpa_process_work_warning  $filtro"); // Total number of articles in the database
        ...
        while( $result = $select->fetch_array(MYSQLI_ASSOC) ) {
            ...
            $articleList .= ' 
            <tr>
            <td class="text-left"><a >' .$result['id_rpa_process_work_warning'].'</a></td> 
