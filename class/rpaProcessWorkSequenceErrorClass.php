 ...
 * Time: 10:58
 */
include 'rpaProcessWorkSequenceErrorClassImpl.php';
class rpaProcessWorkSequenceErrorClass implements rpaProcessWorkSequenceErrorClassImpl
{
...
private $per_page;
    ...
    public function Paginador()
    {
        ...
        $db=new conexion();
        $filtro='';
            $filtro.='WHERE id_rpa_process_work_error= '.$this->getId();

        if ($this->getPage() != 1) $start = ($this->getPage()-1) * $this->getPerPage();
        else $start=0;

        $select = $db->query("SELECT * FROM rpa_process_work_error $filtro LIMIT $start, $this->per_page "); // Select article list from $start

        $numArticles = $db->query("SELECT id FROM rpa_process_work_error  $filtro"); // Total number of articles in the database
        ...
        while( $result = $select->fetch_array(MYSQLI_ASSOC) ) {
            ...
            $articleList .= ' 
            <tr>
            <td class="text-left"><a >' .$result['id_rpa_process_work_error'].'</a></td> 
